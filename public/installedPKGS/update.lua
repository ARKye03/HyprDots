...
local warp_update = pacman ..
    ...
    pacman_warp_conf .. " && " .. yay .. " -Syu --noconfirm --config " .. pacman_warp_conf

local function updatePackages()
    os.execute(pkgsScript)
    os.execute("git add " .. pkgsScript)
...
local function perform_update(use_warp)
    ...
    if success and (exitType == "exit" and exitCode == 0) then
        ...
        print(updateType ..
            " update failed with exit reason: " .. tostring(exitType) .. ", exit code: " .. tostring(exitCode))
    end
end
